...
namespace Clinica_DSII.dashboard.horarios
{
    public partial class create : System.Web.UI.Page
    {
        ...
        {
            TimeSpan horai, horafin;

            horai = TimeSpan.Parse(txtHoraI.Text);
            horafin = TimeSpan.Parse(txtHoraF.Text);

            Horario hr = new Horario(horai,horafin);
            if (dao.insertar(hr)) {
                Response.Redirect("/dashboard/horarios/create");            
            }else
            {
                lblr.Text = hr.horafinal.ToString();
            }
        }
    }
}